...
const server = http.createServer((req, res) => {
    ...
    //    GET Teacher 
    if (req.method === "GET" && req.url === "/teachers") {
    ...
    //    GET Teacher 
    if (req.method === "GET" && req.url === "/stafrolls") {
    ...
    //    POST student 
        ...
        req.on("end", () => {
            ...
            students.forEach(student => {
                database.query(
                    "INSERT INTO student(student_name, roll_no ,date_of_birth, gender ,contact_no, email ,address , institute_id, class_id , attendance_id) VALUES ($1,$2,$3,$4, $5, $6, $7, $8, $9 )",
                    [student.student_name, student.roll_no, student.date_of_birth, student.gender, student.contact_no, student.email, student.address, student.institute_id, student.class_id, student.attendance_id],
                    (err) => {
                        if (err) {
                            res.writeHead(500);
    ...
    // POST teachers 
    else if (req.method === "POST" && req.url === "/teacher") {
        ...
        req.on("end", () => {
            const teachers = JSON.parse(body);
            ...
            teachers.forEach(teacher => {
                database.query(
                    ...
                    (err) => {
                        if (err) {
                            res.writeHead(500);
                            res.end("Error saving teachers");
                        } else {
                            res.writeHead(201);
                            res.end("Teacher saved successfully");
                        }
                    }
                );
            });
    ...
    // create staff role
    else if (req.method == "POST" && req.url == "/staffrole") {
        let body = " ";
            ...
            roles.forEach(role => {
                database.query(
                    ...
                    (err) => {
                        if (err) {
                            res.writeHead(500);
                            res.end("Error saving staff role");
                        } else {
                            res.writeHead(201);
                            res.end("staff role saved successfully");
                        }
                    }
                );
            })
    ...
    // create staffs
    else if (req.method == "POST" && req.url == "/staff") {
        let body = " ";
        ...
        req.on("end", () => {
            const staffs = JSON.parse(body);

            staffs.forEach(staff => {
                database.query(
                    "INSERT INTO staff (staff_name, role_id) VALUES ($1, $2)",
                    [staff.staff_name, staff.roll_id],
                    (err) => {
                        if (err) {
                            res.writeHead(500);
                            res.end("Error saving staffs");
                        } else {
                            res.writeHead(201);
                            res.end("staffs saved successfully");
                        }
                    }
                );
            })
   ...
   // create teacher attendance
   else if (req.method == "POST" && req.url == "/teacher-attendance") {
    let body = " ";
    ...
    req.on("end", () => {
        ...
        data.forEach(attendance => {
            database.query(
                "INSERT INTO teacher_attendance ( teacher_id, attendance_date, status) VALUES ($1, $2, $3)",
                [attendance.teacher_id, attendance.attendance_date, attendance.status],
                (err) => {
                    if (err) {
                        res.writeHead(500);
                        res.end("Error saving teacher attendance");
                    } else {
                        res.writeHead(201);
                        res.end("teacher attendance saved successfully");
                    }
                }
            );
        })
...
// create staff attendance
else if (req.method == "POST" && req.url == "/staff-attendance") {
    let body = " ";
    ...
    req.on("end", () => {
        ...
        data.forEach(attendance => {
            database.query(
                "INSERT INTO staff_attendance ( staff_id, attendance_date, status) VALUES ($1, $2, $3)",
                [attendance.staff_id, attendance.attendance_date, attendance.status],
                (err) => {
                    if (err) {
                        res.writeHead(500);
                        res.end("Error saving staff attendance");
                    } else {
                        res.writeHead(201);
                        res.end("staff attendance saved successfully");
                    }
                }
            );
        })
...
// create student attendance
else if (req.method == "POST" && req.url == "/student-attendance") {
    let body = " ";
    ...
    req.on("end", () => {
        ...
        data.forEach(attendance => {
            database.query(
                "INSERT INTO student_attendance ( student_id, attendance_date, status) VALUES ($1, $2, $3)",
                [attendance.student_id, attendance.attendance_date, attendance.status],
                (err) => {
                    if (err) {
                        res.writeHead(500);
                        res.end("Error saving student attendance");
                    } else {
                        res.writeHead(201);
                        res.end("student attendance saved successfully");
                    }
                }
            );
        })
