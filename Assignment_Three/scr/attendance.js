...
const server = http.createServer((req, res) => {
                ...
                database.query(
                    ...
                    (err) => {
                        if (err) {
                            ...
                            res.writeHead(201);
                            res.end("teachers saved successfully");
                        }
                    }
                );
            });
    ...
    // create staff roll
    else if (req.method == "POST" && req.url == "/staffroll") {
        let body = " ";
        ...
        req.on("end", () => {
            const rolls = JSON.parse(body);

            rolls.forEach(roll => {
                database.query(
                    "INSERT INTO staff_roll (roll_name) VALUES ($1)",
                    [roll.roll_name],
                    (err) => {
                        if (err) {
                            res.writeHead(500);
    ...
    // create staffs
        ...
        req.on("end", () => {
            ...
            staffs.forEach(staff => {
                database.query(
                    "INSERT INTO staff (staff_name, roll_id) VALUES ($1, $2)",
